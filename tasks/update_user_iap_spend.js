...
const resetIAPSpend = async () => {
  await User.updateMany({}, { iap_spend: 0 });
};

const updateIAPSpend = async () => {
  let total = 0;
  ...
  for (const key in updateArr) {
    await User.findByIdAndUpdate(key, { $inc: { iap_spend: updateArr[key] } });
